...
import { fetchFeeds, createNewFeed, like, removeLike, deletePost } from '../services/backendClient' 
...
const delteFeedSuccess = (id) => ({
  type: DELETE_FEED_SUCCESS,
  id
})

const delteFeedRequest = () => ({
  type: DELETE_FEED_REQUEST
})

export const deleteFeed = (id) => async (dispatch) => {
  dispatch(delteFeedRequest())
  try {
    await deletePost(id)
    dispatch(delteFeedSuccess(id))
  }catch(err) {
    dispatch(delteFeedFail())
  }
}
